	...
	for command in trunk_template:
		if command.endswith('allowed vlan'):
			if fast_int['trunk'][intf][0] == 'add':
				add = ', '.join(fast_int['trunk'][intf][1:])
				print(' switchport trunk allowed vlan add {}'.format(add))
			elif fast_int['trunk'][intf][0] == 'only':
				only = ', '.join(fast_int['trunk'][intf][1:])
				print(' switchport trunk allowed vlan {}'.format(only))
			elif fast_int['trunk'][intf][0] == 'del':
				rem = ', '.join(fast_int['trunk'][intf][1:])
				print(' switchport trunk allowed vlan remove {}'.format(rem))
		else:
			print(' {}'.format(command))
