...
topology_filename = input('Enter topology name: ')
list_of_files = glob.glob('sh_cdp_n_*')


def generate_topology_from_cdp(list_of_files, topology_filename, save_to_file=True):
    topology = {}
    for file in list_of_files:
        with open(file) as src:
            file_str = src.read()
            topology.update(task_17_2.parse_sh_cdp_neighbors(file_str))
    if save_to_file:
        with open(topology_filename, 'w') as dest:
            yaml.dump(topology, dest)
    return topology

if __name__ == '__main__':
    generate_topology_from_cdp(list_of_files, topology_filename)
